...
namespace YRender {
	...
	bool GLTexture::SetImg(const Image& img) {
		...
		glTexImage2D(GL_TEXTURE_2D, 0, outerFormat, img.GetWidth(), img.GetHeight(), 0, outerFormat, GL_FLOAT, img.GetData());
		...
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT); //GL_CLAMP_TO_EDGE 默认先都repeat
